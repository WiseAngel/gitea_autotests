...
## Дополнение: переход на официальный `qase-pytest`

### Что изменилось
- Принято решение перейти с самописного TMS-отправителя на официальный `qase-pytest`
- `qase-pytest` установлен в проект и доступен в локальной venv
- Тесты теперь помечаются `@qase.id(...)`
- CI переведён на переменные:
  - `QASE_MODE=testops`
  - `QASE_TESTOPS_PROJECT=GITEA`
  - `QASE_TESTOPS_API_TOKEN`
- Самописный `scripts/tms_reporter.py` остаётся как резервный инструмент, но основной путь теперь официальный

### Проверка
- `from qase.pytest import qase` импортируется успешно
- `pytest --collect-only` по компонентным тестам работает с новыми маркерами

### Итог
- Qase теперь используется по рекомендованному паттерну
- Результаты будут отправляться через официальный reporter, а не через custom POST-скрипт
