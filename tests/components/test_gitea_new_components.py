...
@qase.id(12)
@allure.title("Explore repos page renders a visible search input")
...
@qase.id(13)
@allure.title("Submitting the explore search form navigates to a filtered results URL")
@allure.severity(allure.severity_level.CRITICAL)
def test_explore_repos_search_returns_results(page: Page) -> None:
    ...
    with allure.step("Open the explore repos page"):
        page.goto(f"{settings.base_url}/explore/repos")

    with allure.step(f"Search for '{_SEED_REPO}'"):
        search = GiteaSearchComponent(page)
        search.search(_SEED_REPO)

    with allure.step("Verify the URL reflects the query and results render"):
...
@qase.id(14)
@allure.title("Seeded public repository card is visible in explore results")
@allure.severity(allure.severity_level.CRITICAL)
...
@qase.id(15)
@allure.title("Public user profile layout renders for the seed admin user")
@allure.severity(allure.severity_level.CRITICAL)
...
@qase.id(16)
@allure.title("Issue list component renders on the seeded public repo")
@allure.severity(allure.severity_level.CRITICAL)
...
@qase.id(17)
@allure.title("New-issue form renders for an authenticated user")
@allure.severity(allure.severity_level.CRITICAL)
def test_new_issue_form_is_visible_after_login(authenticated_page: Page) -> None:
    ...
    with allure.step("Navigate to the new-issue page for the seed repo"):
        authenticated_page.goto(f"{settings.base_url}/{_SEED_OWNER}/{_SEED_REPO}/issues/new")

    with allure.step("Verify the new-issue form is visible"):
